...
Their nutritional contents with requirements per dish are

| Nutrition            | Carrot | Cabbage | Cucumber | Requirement |
|----------------------|-------:|--------:|---------:|------------:|
| Vitamin A (mg/kg)    | 35     | 0.5     |      0.5 | 0.5mg       |
| Vitamin B (mg/kg)    | 60     | 300     |      10  | 15mg        |
| Dietary Fibre (g/kg) | 30     | 20      |      10  | 5g          |

1. Create variables corresponding to each ingredient. E.g., `pyo.Var(within=pyo.NonNegativeReals)`.
2. Create an objective function to minimise the total cost. E.g., `pyo.Objective(expr=..., sense=pyo.minimize)`.
3. Create constraints for each of the nutritional requirements. E.g., `pyo.Constraint(expr=...)`.
4. Solve the linear program. E.g., `solver = pyo.SolverFactory('glpk') ; results = solver.solve(model, tee=True) ; model.pprint()`. Does the solution match that of the YouTube video (carrot = 9.5g, cabbage = 38g, cucumber = 290g)?
5. Let's suppose that cucumbers aren't available. There are a couple of ways we could achieve this - for example, we could rebuild with the third column removed. Alternatively, we could add an equality constraint on the problem (cucumber = 0).
...
4. How can you use this form to condense the creation of all the constraints? Solve the problem and check that your results match your previous results.
