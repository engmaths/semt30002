...
$$
\left.\frac{d^2 u}{dx^2}\right|_{x=a} = \frac{2 u(a) + C u(a+\Delta x) + 4u(a + 2 \Delta x) - u(a + 3\Delta x)}{(\Delta x)^2}
$$
...
2. Implement a version of Euler's method (with `for` loops or using vectorisation) that can solve an arbitrary system of ODEs of the form
$$
\renewcommand{\vec}[1]{\boldsymbol{#1}}
\frac{d\vec{u}}{dt} = \vec{f}(\vec{u}, t, \mu),
$$
...
$$
\frac{d u_1}{dt} = -\omega u_2, \qquad 
\frac{d u_2}{dt} = \omega u_1, \tag{1}
$$
...
\begin{align}
c \frac{d T_1}{dt} &= I^2 R - h(T_{1} - T_\text{air}) - h(T_{1} - T_{2}), \tag{2} \\
 c \frac{d T_i}{dt} &= I^2 R - h(T_{i} - T_{i-1}) - h(T_{i} - T_{i+1}), \quad i = 2, 3, \ldots, N - 1, \tag{3} \\
c \frac{d T_N}{dt} &= I^2 R - h(T_{N} - T_\text{air}) - h(T_{N} - T_{N-1}). \tag{4}
\end{align}
