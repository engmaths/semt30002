...
* [PDF of demos](demos/Week03-Demos.pdf)
...
2. Explicit vs implicit Euler.  In this exercise, you will compare the performance of the two Euler methods.  Consider the diffusion equation and boundary conditions from Exercise 1.  Take $a = 0$, $b = 1$, and $D = 0.1$.  Discretise the spatial domain using 31 grid points ($N = 30$) and solve the problem until time $T = 1$.
    ...
    (b) Increase the time step to $\Delta t = 0.1$ and solve with the implicit Euler method.  How does the speed of the implicit Euler method now compare to the speed of the explicit Euler method with $\Delta t = 0.005$?  Which of the two methods leads to a more accurate solution when $t = 1$?  Find out more about how to time your code [here](timing_code.qmd).
